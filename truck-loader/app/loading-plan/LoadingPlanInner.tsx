    ...
    <div className="flex flex-col h-[calc(100vh-68px)] overflow-hidden">
      ...
      {activeView === 'schedule' && (
        <div className="flex-1 overflow-y-auto p-5">
          ...
          {/* 計画週セレクター */}
          <div className="bg-white rounded-lg border border-slate-200 shadow-sm mb-5 px-5 py-3 flex items-center justify-between gap-4">
                  ...
                  <thead>
                    <tr className="bg-slate-50 border-b-2 border-slate-200">
                      ...
                      {weekDates.map((date, i) => (
                        <th key={i} className="px-2 py-2.5 text-center font-semibold min-w-[80px]"
                      ...
                      <th className="px-3 py-2.5 text-center font-semibold text-slate-400 min-w-[52px]">日数</th>
                    </tr>
                  </thead>
                  <tbody>
                    {scheduleWarehouses.length === 0 ? (
                      <tr><td colSpan={9} className="px-4 py-8 text-center text-slate-400 italic">配分比率が設定された拠点がありません</td></tr>
                    ) : (
                              ...
                              return (
                                <td key={dayIdx} className="px-1 py-1.5 text-center">
                                  <button
                                    ...
                                    className={clsx(
                                      'w-16 rounded-lg border transition-all flex flex-col items-center justify-center gap-0.5 px-1 py-1.5 mx-auto',
                                      active ? 'border-blue-400 bg-blue-50' : 'border-slate-200 bg-white hover:border-slate-400',
                                    )}
                                    style={{ minHeight: hasQty ? 56 : 36 }}
                                  >
                                    {active ? (
                                      <>
                                        ...
                                        {hasQty && (
                                          <>
                                            ...
                                            <span className="text-[9px] font-semibold text-slate-700">{cellPlan.totalQty.toLocaleString()}個</span>
                                          </>
                                        )}
                                        {active && !hasQty && <span className="text-[9px] text-blue-300">計画なし</span>}
                                      </>
                                    ) : (
