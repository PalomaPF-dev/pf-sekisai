    ...
    <div className="flex flex-col md:h-[calc(100vh-68px)] md:overflow-hidden">
      ...
      {activeView === 'schedule' && (
        <div className="flex-1 md:overflow-y-auto p-3 md:p-5">
          ...
          {/* 説明バナー */}
          <div className="mb-4 px-4 py-3 bg-indigo-50 border border-indigo-200 rounded-lg text-xs text-indigo-800 flex items-start gap-2">
            <span className="text-base leading-none shrink-0">💡</span>
            <div className="leading-relaxed">
              <strong>スケジュール設定と積載計画の関係：</strong>
              各拠点の<strong>週間送り数（パレット）を出荷日数で均等分割</strong>して、1日あたりの積載量を算出します。<br />
              例：週間送り数 12P・出荷2日 → <span className="font-bold text-indigo-700">1日あたり 6P</span>　／　週間送り数 12P・出荷4日 → <span className="font-bold text-indigo-700">1日あたり 3P</span><br />
          ...
          {!factory ? (
            ...
            <>
                    ...
                    <tr className="bg-slate-50 border-b-2 border-slate-200">
                      ...
                      {weekDates.map((date, i) => (
                        <th key={i} className="px-2 py-2.5 text-center font-semibold min-w-[76px]"
                          style={{ color: i === 5 ? '#4f46e5' : i === 6 ? '#dc2626' : '#6b7280' }}
                        >
                          <div className="font-bold text-[13px]">{DAY_LABELS[i]}</div>
                          <div className="text-[10px] font-normal" style={{ color: i === 5 ? '#a5b4fc' : i === 6 ? '#fca5a5' : '#9ca3af' }}>
                            {formatMD(date)}
                          </div>
                        </th>
                      ))}
                                ...
                                <td key={dayIdx} className="px-1 py-1.5 text-center">
                                  <button
                                    ...
                                    className={clsx(
                                      ...
                                      active ? 'border-indigo-400 bg-indigo-50' : 'border-slate-200 bg-white hover:border-slate-400',
                                    )}
                                  ...
                                  >
                                    {active ? (
                                      <>
                                        ...
                                        {hasQty ? (
                                          <>
                                            <span className="text-[9px] font-bold text-indigo-700">{cellPlan.trucks.length}台</span>
                                          ...
                                          <span className="text-[9px] text-indigo-300">計画なし</span>
                                        )}
                                      </>
                                    ) : (
              ...
              <div className="mt-3 flex items-center gap-4 text-[11px] text-slate-400">
                <span className="flex items-center gap-1"><span className="inline-block w-3 h-3 rounded border border-indigo-400 bg-indigo-50" /> 出荷あり（クリックで解除）</span>
                ...
                <>
                  <div className="mb-4">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      ...
                      {effectiveDay >= 0 && (
                        <span className={clsx('text-[10px] font-bold px-1.5 py-0.5 rounded', effectiveDay === 5 ? 'bg-indigo-100 text-indigo-700' : effectiveDay === 6 ? 'bg-red-100 text-red-600' : 'bg-slate-100 text-slate-600')}>
