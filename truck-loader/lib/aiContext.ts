...
export interface AiContextPayload {
  ...
  plans: {
    warehouse: string; dockTruck: string; trucksUsed: { type: string; count: number }[];
    trucks: number; totalPallets: number; totalQty: number; fillRatePct: number;
  }[];
...
export const aiContextPayloadSchema = z.object({
  ...
  plans: z.array(z.object({
    warehouse: z.string(), dockTruck: z.string(),
    trucksUsed: z.array(z.object({ type: z.string(), count: z.number() })),
    trucks: z.number(),
...
export function buildAiContext(input: AiContextInput): AiContextPayload {
  ...
  const plans = Object.entries(allPlans)
    .map(([name, plan]) => {
      ...
      // エンジンが選定した実際のトラック構成（車種ごとの台数）
      const byType: Record<string, number> = {};
      for (const t of plan.trucks) byType[t.truckTypeCode] = (byType[t.truckTypeCode] ?? 0) + 1;
      const trucksUsed = Object.entries(byType).map(([type, count]) => ({ type, count }));
      return {
        warehouse: name,
        dockTruck: firstWh?.truckType ?? '', // 受入可能な最大トラック（ドック制約）
        trucksUsed,
        trucks: plan.trucks.length,
        totalPallets: plan.totalPallets,
        totalQty: plan.totalQty,
        fillRatePct: fillRate(plan),
      };
    })
    .filter((p) => p.trucks > 0);
...
- 計算エンジンは既に各拠点の総パレット数から最適なトラック構成を「選定済み」です（plans[].trucksUsed = 選定された車種ごとの台数、plans[].fillRatePct = 積載率）。plans[].dockTruck は受入可能な最大トラック（ドック制約）です。
- 積み付けは「重量物・上積み不可(allowStackOnTop=false)の製品を下段、軽量で積み重ね可(stackable)の製品を上段」を原則とし、その上にあるものは積まないでください。
- トラック選定の助言では、選定済み構成(trucksUsed)を踏まえ、なお積載率が低い拠点について「出荷曜日の統合（複数日を1便に）」「拠点間の積み合わせ」「dockTruck制約の見直し」など、効率を上げる現実的な代替案を理由つきで提案してください。
