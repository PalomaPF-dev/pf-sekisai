...
4. 商品を2つ作成（**確定価格・会社単位/人数無制限**）:
   - 月額: product id `pro_monthly`／**¥2,980/月**（自動更新）
   - 年額: product id `pro_annual`／**¥29,800/年**（自動更新・約2ヶ月分お得）
   - **無料トライアル: 7日間**（各商品の「導入オファー」→「無料」7日を設定）
5. 各商品の審査用情報・スクリーンショット（購入画面=設定→プランのスクショ）を登録
...
7. **Integrations → Webhooks** を追加:
   - URL: `https://smakouba-truck-loader.vercel.app/api/webhooks/revenuecat`
...
NEXT_PUBLIC_SYNC_API=https://smakouba-truck-loader.vercel.app \
npm run build:ios
